...
(function($) {
	function TouchCarousel(element, options) {	
		...
		if(this.settings.directionNav) {	
			this._itemsWrapper.after("<a href='#' class='arrow-holder left'><i class='fa fa-caret-left'></i></a> <a href='#' class='arrow-holder right'><i class='fa fa-caret-right'></i></a>");
	...
	TouchCarousel.prototype = {
			...
			updateCarouselSize:function(leavePos) {
				...
				/*SIC start*/
				if(this.settings.resizeTo){
						var curWidth = 0;
						var cWidth = parseInt(this.carouselWidth*this.settings.resizeToPercent);
						$(this.items).each(function(){
							this.width = cWidth;
							this.item.css('width', cWidth);
							this.posX = curWidth;
							curWidth += this.width;
						});
						this._totalItemsWidth = curWidth;
						this._dragContainer.css('width', curWidth)
						this._maxXPos = this._totalItemsWidth;
				}
				/*SIC end*/
